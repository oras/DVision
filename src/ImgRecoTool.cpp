...
Node* ImgRecoTool::getSaliencyMap(Mat &srcImg, int scan){
    Node *head=NULL;
    int **jP;
    Mat binaryImage;
    ...
    threshold(getImageChannel(srcImg,2),binaryImage,135,255,0);
            ...
            for(int c=0;c<binaryImage.cols;c++){
                if(binaryImage.at<uchar>(r,c)==255){
                    ...
                    if(jP==NULL){
                        ...
                        drawSaliencyDatas(head,tempImage);
                        imshow("RGB image", tempImage);
